...
def test_in_sweden(session, search_url, abroad):
    """
    ...
    """
    response = get_search(session, search_url, {ABROAD: abroad})
...
def test_abroad_and_local(session, search_url, geo_param):
    """
    ...
    hits_abroad = get_search(session, search_url, params_abroad)['total']['value']

    hits_geo_only = get_search(session, search_url, geo_param)['total']['value']
    params_abroad.update(geo_param)
    hits_geo_and_abroad = get_search(session, search_url, params_abroad)['total']['value']
...
def test_abroad_and_other_country(session, search_url, geo_param):
    """
    ...
    """
    params_abroad = {ABROAD: True}
    hits_abroad = get_search(session, search_url, params_abroad)
    check_ads_for_country_in_address(hits_abroad['hits'], abroad=True)

    params_abroad.update(geo_param)
    hits_abroad_plus_country = get_search(session, search_url, params_abroad)
...
def test_abroad_false_does_not_affect_results(session, search_url, geo_param):
    """
    ...
    """
    hits_1 = get_search(session, search_url, geo_param)['total']['value']
    geo_param.update({ABROAD: False})
    hits_2 = get_search(session, search_url, geo_param)['total']['value']
