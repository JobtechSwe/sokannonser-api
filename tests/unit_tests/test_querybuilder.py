...
def test_querystring_char_removal(querystring, expected):
    pbquery = QueryBuilder(mock.MockTextToConcept())
    formatted = pbquery._remove_unwanted_chars_from_querystring(querystring)
...
def test_region_municipality_query(args, expected_pos, expected_neg):
    ...
    if expected_neg:
        ...
        for e in expected_neg:
            ...
            def test_extract_querystring_different_quotes(querystring, expected_phrase, expected_returned_query,
                                                          test_id):
                """
