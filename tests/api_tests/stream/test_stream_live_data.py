...
def test_stream_filter_on_date_interval(session_stream, stream_url):
    params = {'date': DAWN_OF_TIME, UPDATED_BEFORE_DATE: current_time_stamp}
    list_of_ads = get_stream(session_stream, stream_url, params)
...
def test_snapshot(session_stream, stream_url):
    ...
    """
    response = get_snapshot_check_number_of_results(session_stream, stream_url, expected_number=None)
