...
def test_name_and_collection_param( session, collection_id, concept_ids):
    """
    ...
    result_json_name = get_search(session,  params)
    ...
    result_json_collection = get_search( session, params)
...
@pytest.mark.slow
@pytest.mark.parametrize("collection_id, concept_ids", get_occupation_collection_id_and_concept_ids())
def test_name_and_collection_param_compare_ids( session, collection_id, concept_ids):
    """
    Fetch collections from Taxonomy and test them one by one
    ...
    for offset in range(0, number_of_ads_collection, limit):
        ...
        json_response_coll = get_search(session,  params)
...
@pytest.mark.slow
def test_name_and_collection_param_multiple_collections( session):
    """
...
def test_empty_collection( session):
...
def test_collection_and_freetext( session):
    """
    Fetch collections from Taxonomy and test them one by one
...
def test_all_collections( session):
    """
    ...
    r = get_search(session,  params)


def test_plus_minus( session):
    """
